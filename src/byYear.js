...
const ByYear = () => {
  ...
  useEffect(() => {
    ...
    const showTooltip = (event, year, data) => {
      ...
      const totalCasualties = yearData.reduce((sum, item) => sum + item.Casualties, 0);
...
// Your existing code to create the pie chart...
  ...
  .each(function (d) {
    ...
    d3.select(this.parentNode)
      .on('mouseover', function (event, d) {
        const translationX = arc.centroid(d)[0] * 0.5;
        const translationY = arc.centroid(d)[1] * 0.5;
        d3.select(this)
          .transition()
          .duration(250)
          .attr('transform', `translate(${translationX}, ${translationY}) scale(1.1)`)
          .attr('stroke', 'black')
          .attr('stroke-width', 0.5);

        label.text(`${d.data.group}: ${d.data.length}`)
          .transition()
          .duration(250)
          .style('opacity', 1);
      })
      ...
      svg.append("text")
        ...
        .style("font-size", "18px")
        .style("text-decoration", "underline")
        .text("Target Types of the Terrorist Attacks");
